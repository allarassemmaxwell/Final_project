									...
									<tr>
										...
										<td> 
											...
											<form action="" method="POST" style="margin-left:-40px;">
												...
												<button name="delete-admin-expense">
											...
											<div style="margin-left:30px; margin-top:-25px">
												<button>
													<a href="expense-update.php?id1=<?php echo $row['user_id'] ?>&id2=<?php echo $row['expense_id'] ?>&id3=<?php echo $product_service_data['product_service_id'] ?>&price=<?php echo $row['price'] ?>">
				...
				<form name="expenseForm" method="POST" onsubmit="return expenseValidation()">
					...
					<div>
						<select id="income" name="income"  style="font-size: 14px; color: #737373;padding-left: 5px; padding-right: 5px;">
							...
							<?php 
								$income_query   = "SELECT * FROM Income WHERE remaining_amount > 1 ORDER BY created_at DESC";
								$income_result = mysqli_query($con, $income_query);
					...
					<div>
						<select id="product_or_service" name="product_or_service"  style="font-size: 14px; color: #737373; padding: 10px;">
							...
							<?php 
								...
								if (mysqli_num_rows($results) > 0) {
									while($product_row = mysqli_fetch_assoc($results)) {
										echo '<option value="' . $product_row['product_service_id'] . '">' . $product_row['name'] . '</option>';
									}
								}
							?>
						</select>
