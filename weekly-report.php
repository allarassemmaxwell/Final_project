<?php 
	...
	if (isset($_POST['filterByDate'])) {
		...
		$the_week = date("YW",strtotime($projected_date)) -1;
	}
?>
		...
		<div class="main-content">
			...
			<div style="overflow-x:auto;" id="tblCustomers" cellspacing="0" cellpadding="0">
				<div class="report-title">
					<div style="font-size:18px; text-align:center;">Weekly Report</div>
					<div class="report-filter" onclick="shoFilterByWeek()" style="float: right; margin-top: -35px; font-size: 15px;">
						<button style=" color: #00C2FF; height: 30px;  cursor: pointer; border: 1px solid #00C2FF;">Filter by Week</button>
					</div>
					<form name="filterByForm" method="POST" onsubmit="return filterByValidation()">
						<div id="filter">
							<input style="font-size: 14px; color: #737373;padding: 25px; height: 15px;" value="<?php echo $projected_date; ?>" type="week" name="projected_date" id="projected_date">
				...
				<table style="margin-top: 15px;">
					<?php
						$total_expenditure = 0;
						$query1 = "SELECT * FROM Expense WHERE user_id = '$user_id' AND YEARWEEK(created_at) = '$the_week' ORDER BY created_at DESC";
				...
				<?php 
					$remaining = 0;
					$income    = 0;
					$remaining_query  = "SELECT * FROM Income WHERE user_id = '$user_id' AND YEARWEEK(created_at) = '$the_week' ORDER BY created_at DESC";
